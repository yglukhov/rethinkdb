...
proc hmac_sha256_prf(keyPtr: pointer, keyLen: uint32,
         textPtr: pointer, textLen: uint32,
         randomPtr: pointer) {.cdecl.} =
    let r = hmac_sha256(stringWithData(keyPtr, keyLen), stringWithData(textPtr, textLen))
    for i, b in r:
       cast[ptr char](cast[uint](randomPtr) + i)[] = b
...
proc pbkdf2_hmac_sha256*(hLen: uint32, password, salt: string, iter: uint32): string =
    result = newString(hLen.int)
    var tempBuf = newString(int(max(hLen, uint32(salt.len + 4)) + 2'u32 * hLen))
    pbkdf2(hmac_sha256_prf, hLen, password, salt, iter, result.cstring, result.len.uint32, tempBuf.cstring)
